...
\null \quad O banco de dados relacional desenvolvido segue o \acrfull{DER} da Figura \ref{DER} e foi implementado no \acrfull{SGBD} PostgreSQL por ser um software livre e com capacidade para lidar com o volumes de dados produzido pelo ambiente \acrshort{IoT} em que o sistema SenseHera está inserido. A Figura \ref{MR} mostra o Modelo Relacional implementado.

\figura[!h]{tccDER.jpg}{\acrlong{DER}}{DER}{scale=0.5}
\newpage
\figura[!h]{tccMR.jpg}{Modelo Relacional implementado}{MR}{scale=0.5}
\\\null \quad A entidade Respostas\_Possíveis foi criada com o intúito de evitar problemas durante a fase de colaboração humana com o sistema. Ao fixar as possibilidades de respostas para cada pergunta, evita-se problemas como por exemplo, erros de ortografia e erros na compreensão da pergunta.
...
\figura[!h]{fluxogramaSensor.jpg}{Fluxograma do funcionamento básico de um dispositivo sensitivo construído}{fluxogramaSensor}{scale=0.57}
\newpage
...
\\\null\quad A Figura \ref{mensagemJSONExemplo} mostra um exemplo de uma mensagem inicial enviada de um dispositivo sensitivo para o sistema SenseHera. A Figura \ref{mensagemJSONExemplo2} mostra um exemplo de mensagem subsequente à mensagem inicial, neste caso, ao receber a primeira mensagem, o SenseHera criou uma instância da entidade Sensor cujo identificador é igual à 5 e enviou esse código para o dispositivo.
\\\\\\
\figura[!h]{mensagemJSON.jpg}{Formato genérico para a mensagem JSON}{mensagemJSON}{scale=0.6}
\pagebreak
\figura[!h]{mensagemJSONExemplo.jpg}{Exemplo de uma mensagem inicial em formato JSON}{mensagemJSONExemplo}{scale=0.6}
\figura[!h]{mensagemJSONExemplo2.jpg}{Exemplo de uma mensagem subsequente à mensagem inicial em formato JSON}{mensagemJSONExemplo2}{scale=0.6}
